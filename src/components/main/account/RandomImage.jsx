...
function RandomImage({ header, genreId, genreName }) {
  ...
  // Determine the selected data based on the header
  const selectedData =
    header === "Movies" ? topM : header === "TV Shows" ? topTv : data;
  ...
  // Filter data by genre, ensuring it is always an array
  const filterDataByGenre = (data, genreId) => {
    return genreId
      ? data.filter((item) => item?.genre_ids?.includes(genreId))
      : data;
  };

  // Set a random image from the filtered data
    ...
    <>
      ...
      <div className="block xs:hidden w-[90%] mx-auto">
        <div className="w-full pt-4">
          ...
          {randomImage && (
            <img
              className="object-cover min-h-[440px] w-full h-[490px] rounded-xl"
              src={`https://image.tmdb.org/t/p/original${randomImage.backdrop_path}`}
              alt="Mobile Preview"
            />
          )}
