  ...
  return (
    <>
      ...
      <div className="hidden xs:block max-w-[1600px] mx-auto overflow-hidden">
        ...
        <div
          className={`relative min-h-[150px] flex items-center transition-all duration-500 ${
            isSwipedRight ? "pl-0" : "pl-10"
          }`}
        >
          <Swiper
            ...
            cssMode={true}
            navigation={{
              nextEl: `.custom-next${customClass}`,
              prevEl: `.custom-prev${customClass}`,
            }}
            keyboard={true}
              ...
              items
                .filter(
                  (item) =>
                    ...
                    item.poster_path // Ensure there's a poster_path
                )
                .map((item) => (
                  <SwiperSlide
                    key={item.id}
                    className="swiper-slide-trend2 cursor-pointer"
                  >
                    <Card
                      type={type}
          ...
          <div className="flex justify-center w-[70px] items-center transform -translate-y-1/2 absolute top-1/2 -left-1 z-20">
            <div
              className={`custom-prev${customClass} flex items-center justify-center h-[131px] w-[65px] bg-gradient-to-r from-[#000000c2] hover:from-[#000000e5] to-transparent text-white cursor-pointer transition-all duration-150`}
            >
          ...
          <div className="w-[70px] flex justify-center items-center transform -translate-y-1/2 absolute top-1/2 -right-1 z-20">
            <div
              onClick={swipeRight}
              className={`custom-next${customClass} flex items-center justify-center h-[131px] w-[65px] bg-gradient-to-l from-[#000000c2] hover:from-[#000000e5] to-transparent text-white cursor-pointer transition-all duration-150`}
            >
      ...
      <div className="xs:hidden w-[90%] mx-auto text-white pt-4 px-2">
        ...
        <Swiper cssMode={true} modules={[Navigation, Keyboard]}>
          {items?.length > 0 ? (
            items
              .filter(
                (item) =>
              ...
              .map((item) => (
                <SwiperSlide
                  key={item.id}
                  className="swiper-slide-trend cursor-pointer"
                >
                  <div
                    onClick={() =>
                      handleSlideMoreInfo(
